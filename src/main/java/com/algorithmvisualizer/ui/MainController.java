...
import com.algorithmvisualizer.util.UpdateManager;
...
public class MainController {
    ...
    @FXML
    private VBox algorithmSection;
    @FXML
    private FlowPane algorithmGrid;
    @FXML
    ...
    private void initialize() {
        ...
        // Set version label
        versionLabel.setText(UpdateManager.getCurrentVersion());
    ...
    private void initializeAlgorithms() {
        allAlgorithms = FXCollections.observableArrayList(
                ...
                new FractionalKnapsackAlgorithm(),
                new DepthFirstSearchAlgorithm(),
                new BreadthFirstSearchAlgorithm(),
                new QueueConceptAlgorithm(),
                new StackConceptAlgorithm(),
                new LinkedListConceptAlgorithmV2(),
                new BinaryTreeConceptAlgorithm(),
                new TowerOfHanoiAlgorithm(),
                new NQueensAlgorithm(),
                new SudokuAlgorithm(),
                new MazeAlgorithm(),
                new GraphColoringAlgorithm(),
                new com.algorithmvisualizer.model.MSTAlgorithm(),
                new com.algorithmvisualizer.model.DijkstraAlgorithm(),
                new com.algorithmvisualizer.model.BellmanFordAlgorithm(),
                new com.algorithmvisualizer.model.HuffmanCodingAlgorithm(),
                new com.algorithmvisualizer.model.LCSAlgorithm(),
                new com.algorithmvisualizer.model.ZeroOneKnapsackAlgorithm());
    ...
    private void createCategoryCards() {
        ...
        categoryColors.put(AlgorithmCategory.GREEDY, "#f1c40f");
        categoryColors.put(AlgorithmCategory.DYNAMIC_PROGRAMMING, "#9c27b0");
    ...
    private void openAlgorithm(Algorithm algorithm) {
        try {
            ...
            stage.setTitle("AlgoBuddy - " + algorithm.getName());
    ...
    private void configureWindowSize() {
        mainContent.sceneProperty().addListener((obs, oldScene, newScene) -> {
            if (newScene != null) {
                System.out.println("=== MAIN CONTROLLER SCENE CHANGE ===");
                System.out.println("New scene attached to mainContent");
                System.out.println("  - Scene width: " + newScene.getWidth());
                System.out.println("  - Scene height: " + newScene.getHeight());

                newScene.windowProperty().addListener((obsWin, oldWin, newWin) -> {
                    if (newWin instanceof Stage) {
                        Stage stage = (Stage) newWin;

                        System.out.println("Window attached to scene:");
                        ...
                        System.out.println("Configuring window size:");
    ...
    private void checkForUpdates() {
        System.out.println("[MainController] Update check button clicked.");
        updateButton.setDisable(true);
        updateButton.setText("Checking...");

        new Thread(() -> {
            try {
                System.out.println("[MainController] Starting update check background thread...");
                UpdateManager.UpdateInfo info = UpdateManager.checkForUpdates();
                System.out.println("[MainController] Update check background thread completed normally.");
                ...
                Platform.runLater(() -> {
                    ...
                    if (info.isUpdateAvailable) {
                        System.out.println("[MainController] Update prompt shown to user: " + info.latestVersion);
                        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
                        alert.setTitle("Update Available");
                        alert.setHeaderText("A new version " + info.latestVersion + " is available!");
                        alert.setContentText("Current version: " + UpdateManager.getCurrentVersion()
                        ...
                        if (result.isPresent() && result.get() == downloadBtn) {
                            try {
                                ...
                                System.err.println("[MainController] Failed to open download link: " + e.getMessage());
                                showError("Could not open download link.");
                            }
                        }
                    } else {
                        System.out.println("[MainController] User is already on the latest version.");
                        Alert alert = new Alert(Alert.AlertType.INFORMATION);
                        alert.setTitle("Up to Date");
                        alert.setHeaderText(null);
                        alert.setContentText(
                                "You are on the latest version (" + UpdateManager.getCurrentVersion() + ").");
                        alert.showAndWait();
                    }
                });
            } catch (Exception e) {
                System.err.println("[MainController] Exception in update check thread: " + e.getClass().getName()
                        + " - " + e.getMessage());
                e.printStackTrace();
                Platform.runLater(() -> {
                    updateButton.setDisable(false);
